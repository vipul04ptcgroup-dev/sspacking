  ...
  return (
    <header className={`sticky top-0 z-40 w-full transition-all duration-300 ${scrolled ? 'bg-white shadow-md' : 'bg-white/95 backdrop-blur-sm border-b border-stone-100'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 p-1.5 sm:p-2 rounded-lg hover:bg-stone-100 transition"
                >
                  <div className="w-7 h-7 sm:w-8 sm:h-8 bg-amber-100 rounded-full flex items-center justify-center">
                    <span className="text-amber-700 font-bold text-xs sm:text-sm">
        ...
        {/* Mobile Search Bar (outside hamburger) */}
        {mobileSearchOpen && (
            ...
            <form onSubmit={handleSearch}>
              <div className="relative">
                ...
                <input
                  ...
                  onChange={e => setSearch(e.target.value)}
                  placeholder="Search products..."
                  className="w-full pl-9 pr-4 py-2.5 text-sm bg-stone-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-400"
                />
              </div>
            </form>
          </div>
        ...
        {menuOpen && (
          <div className="lg:hidden border-t border-stone-100 py-4 space-y-1">
            ...
            <Link href="/products" onClick={() => setMenuOpen(false)} className="block px-3 py-2 text-sm font-medium text-stone-700 hover:bg-amber-50 rounded-lg">Products</Link>
