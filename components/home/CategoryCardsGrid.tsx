...
type CategoryCardsGridProps = {
  categories: Array<Pick<Category, 'id' | 'name' | 'slug' | 'image'>>;
};

export default function CategoryCardsGrid({ categories }: CategoryCardsGridProps) {
  return (
    <div
    ...
    >
      {categories.map((cat) => (
        ...
        >
          <div className="relative overflow-hidden bg-[linear-gradient(145deg,#f8f5ef_0%,#ffffff_52%,#eef2f6_100%)] min-h-[260px] sm:min-h-[250px] lg:min-h-[285px]">
            {cat.image ? (
              <Image
                src={cat.image}
                alt={cat.name}
